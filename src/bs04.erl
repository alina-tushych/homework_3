...
decode(<<"{", Rest/binary>>, Method) ->
    prop(clear_input(Rest, no, <<>>), key, 1, <<>>, [], 
        case Method of 
            proplist -> []; 
            map -> #{} 
        end
...
prop(<<C, Rest/binary>>, Flag, InnerCnt, Bin, List, Res) when (C == 58) or (C == 44) -> 
...
prop(<<"'", Rest/binary>>, Flag, InnerCnt, Bin, List, Res) when (Flag == value) or (Flag == inner) ->
    ...
    case Flag of
        value ->
        prop(NewRest, value, InnerCnt, <<>>, List, 
        case is_map(Res) of 
            false -> [{Bin, Value}|Res]; 
            _ -> maps:put(Bin, Value, Res) 
        end);
    ...
    case (Cnt == 1) of
        ...
        case Flag of
            ...
            case Bin == <<>> of
                ...
                _ -> prop(skip(Rest, Count+1), value, Cnt, Bin, List, 
                case is_map(Res) of 
                    false -> [InnerRes|Res]; _ -> maps:merge(Res, InnerRes) 
                end)
            end
        end;
        _ -> 
...
prop(<<"}", _/binary>>, _Flag, _InnerCnt, _Bin, _List, Res) -> 
    case is_map(Res) of 
        false -> lists:reverse(Res); 
        _ -> Res 
    end;
...
prop(<<"]", Rest/binary>>, _Flag, InnerCnt, Bin, List, Res) ->
    prop(Rest, value, InnerCnt, <<>>, <<>>, 
        case is_map(Res) of 
            false -> [{Bin, lists:reverse(List)}|Res]; 
            _ -> maps:put(Bin, lists:reverse(List), Res) 
        end);
prop(<<C/utf8, Rest/binary>>, value, InnerCnt, Bin, List, Res) ->
    ...
    prop(NewRest, value, InnerCnt, <<>>, List, 
        case is_map(Res) of 
            false -> [{Bin, parse_value(Value)}|Res]; 
            _ -> maps:put(Bin, parse_value(Value), Res) 
        end);
prop(<<>>, _, _, _, _, Res) -> 
        case is_map(Res) of 
            false -> lists:reverse(Res); 
